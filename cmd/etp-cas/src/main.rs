...
async fn main() {
    ...
    match cli.command {
        Commands::Store { file } => {
            ...
            let (hash, size) = cas::store_blob(&data).unwrap_or_else(|e| {
        ...
        Commands::Get { hash, output } => {
            let data = cas::get_blob(&hash).unwrap_or_else(|e| {
        ...
        Commands::Gc { db, verbose } => {
            ...
            let removed = ops::gc_orphan_blobs(&pool, verbose).await;
        ...
        Commands::List => {
            let hashes = cas::list_blob_hashes().unwrap_or_else(|e| {
                eprintln!("error: {e}");
                std::process::exit(1);
            });
            for hash in &hashes {
